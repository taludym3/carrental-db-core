...
const BranchDocumentDetails = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const { data: document, isLoading } = useQuery({
    queryKey: ['branch-document-details', id],
    queryFn: async () => {
  ...
  if (!document) {
    return (
      ...
      <PageHeader
        ...
        action={
          <Button variant="outline" onClick={() => navigate('/branch/documents')}>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="عرض المستند - للقراءة فقط"
        action={
          <Button variant="outline" onClick={() => navigate('/branch/documents')}>
            <ArrowRight className="h-4 w-4" />
            العودة للقائمة
...
export default BranchDocumentDetails;
